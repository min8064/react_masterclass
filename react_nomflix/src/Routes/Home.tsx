...
function Home() {
	const { data: nowPlayingMovie, isLoading: nowPlayingLoading } =
		useQuery<IGetDatasResult>(["movies", "nowPlaying"], getNowMovies);
	const nowMovies = nowPlayingMovie?.results;

	const { data: latestMovie, isLoading: latestLoading } =
		useQuery<IGetDatasResult>(["movies", "latestMovie"], getLatestMovies);
	...
	const { data: upcomingMovie, isLoading: upcomingLoading } =
		useQuery<IGetDatasResult>(["movies", "upcomingMovie"], getUpcomingMovies);
				...
				<>
					<Banner
						bgPhoto={makeImagePath(
							nowPlayingMovie?.results[0].backdrop_path || ""
						)}
					>
